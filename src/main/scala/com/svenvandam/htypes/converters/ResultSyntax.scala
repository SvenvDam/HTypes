...
trait ResultSyntax extends ScalaConverter {
  ...
  implicit class FutureResultOps(f: Future[Result]) {
    def as[T](implicit decoder: RowDecoder[T], ec: ExecutionContext): Future[Iterable[(T, Long)]] = f
      .map(_.as[T])
  }

  implicit class FutureResultScannerOps(f: Future[ResultScanner]) {
    def as[T](implicit decoder: RowDecoder[T], ec: ExecutionContext): Future[Iterable[Iterable[(T, Long)]]] = f
      .map(_.as[T])
  }
}
